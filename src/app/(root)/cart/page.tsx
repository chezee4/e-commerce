    ...
    <section className="w-full ">
     {cartItems.length > 0 ? (
         ...
         <div className="w-full flex flex-col lg:flex-row justify-between gap-8  mt-24 mb-3">
           <ul className="flex flex-col max-w-[700px] mx-auto w-full gap-6">
             {cartItems.map((item) => (
               <CartProduct
                 key={item.id}
                 id={item.id}
           ...
           <div className="w-full max-w-[450px] mx-auto rounded-lg p-5 lg:p-0 bg-[#f3f2f2] lg:bg-transparent">
            ...
            <div className="w-full h-[calc(100vh-100px)] flex flex-col justify-center items-center">
                ...
                <p className=" text-lg text-center mb-10">Your cart is currently empty.</p>
                <Link href="/" className=" bg-zinc-950 text-white py-3 px-10">RETURN TO SHOP</Link>
            </div>
     )}
    </section>
  );
